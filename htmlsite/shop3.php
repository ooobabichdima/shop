...
<body>
      ...
      <div class="bundle-body">
        <div class="bundle-items">
          ...
          <div class="addon">
            ...
            <div class="muted2" style="font-size:13px;">Рекомендуем минимум 2 шт.</div>
          </div>
      ...
      <div class="footer-grid">
        <div>
          ...
          <p style="margin:0; color:var(--muted); font-size:13px; max-width:60ch;">
            Карточка товара: галерея, цена, количество, табы, отзывы, FAQ, тюнинг-пакеты и “с этим покупают”.
          </p>
        </div>
  ...
  <script>
    (function(){
      ...
      // gallery thumbs (demo: replace placeholder)
      ...
      function getSelectedKitIndex(){
        const r = kitRadios.find(x => x.checked);
      ...
      kitBtns.forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          // На проде: отправляешь kit_id в корзину (POST/Fetch) или в PHP-сессию
      ...
      function calcAddonSum(){
        const sum = addonBoxes.reduce((acc, b) => acc + (b.checked ? parseInt(b.dataset.addonPrice,10) : 0), 0);
        if (addonSumEl) addonSumEl.textContent = fmt(sum);
        return sum;
      }

      addonBoxes.forEach(b => b.addEventListener('change', calcAddonSum));
      calcAddonSum();
      ...
      selectRecommended && selectRecommended.addEventListener('click', () => {
        // “Рекомендованный комплект”: АКБ + зарядка + шары (и если есть второй товар по 390 — отметится тоже)
        addonBoxes.forEach(b => b.checked = false);
        addonBoxes.forEach(b => {
          const id = b.dataset.addonId;
          if (id === 'bat_74' || id === 'charger_smart' || id === 'bbs_025') b.checked = true;
        });
        calcAddonSum();
      });

      addBundleToCart && addBundleToCart.addEventListener('click', () => {
        const selected = addonBoxes.filter(b => b.checked).map(b => ({
        ...
        // На проде: отправляешь ids выбранных товаров (addon_product_id) в корзину
        alert('Добавлено товаров: ' + selected.length + '. Сумма добавок: ' + sum + ' грн');
      });

      // Optional: quick “selected kit index” (use later)
      window.getSelectedKitIndex = getSelectedKitIndex;
    })();
  </script>
</body>
</html>
